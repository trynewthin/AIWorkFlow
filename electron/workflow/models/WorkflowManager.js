...
class WorkflowManager {
   ...
   * @param {string} workflowId - 目标工作流ID (如果为空，则复制到同一工作流)
   * @returns {Promise<string|null>} 新节点ID，失败返回null
   */
  async duplicateNode(nodeId, workflowId = null) {
    try {
      ...
      const targetWorkflowId = workflowId || originalNode.workflow_id;

      // 确保目标工作流存在 (如果提供了新的 workflowId)
      if (workflowId) {
        const targetWorkflow = await this.getWorkflow(targetWorkflowId);
        if (!targetWorkflow) {
          throw new Error(`目标工作流 ${targetWorkflowId} 不存在`);
        }
      }
      ...
      const newNodeId = await this.addNode(
        targetWorkflowId,
        originalNode.type,
        originalNode.flow_config, // 复制流程配置
        originalNode.work_config, // 复制运行时配置
        originalNode.order_index + 1 // 插入到原节点之后
      );

      logger.info(`[WorkflowManager] 复制节点 ${nodeId} 到 ${newNodeId} 成功`);
      return newNodeId;
    } catch (error) {
      logger.error(`[WorkflowManager] 复制节点 ${nodeId} 失败: ${error.message}`);
      throw error;
    }
  }
}

// 导出 WorkflowManager 类和一个获取单例的函数
let instance = null;

/**
 * 获取 WorkflowManager 单例
 * @returns {WorkflowManager} WorkflowManager 单例
 */
